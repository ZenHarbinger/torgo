...
//	NameCompletion nameCompletion;
    ...
    public JConsole(InputStream cin, OutputStream cout) {
        ...
        menu.add(new JMenuItem(CUT)).addActionListener((ActionListener) this);
        menu.add(new JMenuItem(COPY)).addActionListener((ActionListener) this);
        menu.add(new JMenuItem(PASTE)).addActionListener((ActionListener) this);

        text.addMouseListener((MouseListener) this);

        // make	sure popup menu	follows	Look & Feel
        UIManager.addPropertyChangeListener((PropertyChangeListener) this);
    ...
    private void invokeAndWait(Runnable run) {
        if (!SwingUtilities.isEventDispatchThread()) {
            try {
                ...
                // shouldn't happen
                org.tros.utils.logging.Logging.getLogFactory().getLogger(JConsole.class).warn(null, e);
            }
        } else {
            run.run();
        }
